...
let token;
const bitlyApiUrl = 'https://api-ssl.bitly.com/v3/shorten';

function buildBitlyUrl(longUrl, alias) {
	return `${bitlyApiUrl}?access_token=${process.env.bitlyAccessToken}&longUrl=${encodeURIComponent(longUrl)}&domain=bendy.works`;
}

function shortenUrl(longUrl, callback) {
    const url = buildBitlyUrl(longUrl);
    https.get(url, res => {
        ...
        res.on('end', () => {
            try {
                ...
                callback(null, buildResponse(longUrl, parsedData.data.url));
            } catch (e) {
...
function buildResponse(longUrl, url) {
    return {
        response_type: "in_channel",
        // mrkdwn: true,
        parse: 'none',
        text: 'Shortened to `' + url + '`'
    }
}
...
function processEvent(event, callback) {
    ...
    shortenUrl(commandText, callback);
}
